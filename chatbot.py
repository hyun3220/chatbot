...
# 1. 페이지 설정 및 블랙 & 오렌지 테마 레이아웃 (CSS 수정)
...
st.markdown("""
    ...
    [data-testid="stChatMessageAssistant"] { border-left: 5px solid #555 !important; }
...
# 4. 답변 생성 로직 (기존 모델 및 프롬프트 설정 유지)
def generate_answer(api_key, vectorstore, query):
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-flash",  # 질문자님 설정 유지
    ...
    system_prompt = (
        ...
        "2. **정확성**: 함수명은 Context와 대조하여 100% 일치할 때만 답변하세요.\n"
        "3. **친절한 가이드**: 사용 상황에 대한 한 줄 설명을 덧붙이세요.\n\n"
        "🚨 [매우 중요 - 답변 구성 순서]:\n"
        "1️⃣ **기능 요약** | 2️⃣ **핵심 API** | 3️⃣ **전체 예시 코드** | 4️⃣ **주의 사항**\n\n"
        "--- [웹 뷰어 스크립트 템플릿] ---\n"
...
# 5. UI 실행부 (정렬 개선)
st.markdown('<div class="main-title">🤖 CLIP Report v5.0 전문 챗봇</div>', unsafe_allow_html=True)
